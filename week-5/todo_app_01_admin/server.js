...
// SPA route: /admin serves the same index.html (client-side routing)
...
// Re-check role from DB on every request (don't trust the JWT alone —
// a revoked admin with a still-valid token must not get admin access).
async function requireAdmin(req, res, next) {
  try {
    const { rows } = await pool.query(
      `SELECT role FROM ${USERS_TABLE} WHERE id = $1 LIMIT 1`,
      [req.user.id]
    );
    ...
    res.status(500).json({ success: false, message: '권한 확인에 실패했습니다.' });
  }
}
  ...
  try {
    const { rows } = await pool.query(
      `SELECT id, email, name, COALESCE(role, 'user') AS role
         FROM ${USERS_TABLE} WHERE id = $1 LIMIT 1`,
      [req.user.id]
    );
    if (rows.length === 0) {
...
// --- Admin routes (require auth + is_admin re-check against DB) ---

app.use('/api/admin', requireAuth, requireAdmin);

// GET /api/admin/users — 전체 유저 + 각자의 todo 수
...
// DELETE /api/admin/users/:id — 유저 + 해당 유저의 todo 전부 삭제 (트랜잭션)
app.delete('/api/admin/users/:id', async (req, res) => {
  ...
  try {
    ...
    await client.query(`DELETE FROM ${TODOS_TABLE} WHERE user_id = $1`, [targetId]);
    const { rowCount } = await client.query(`DELETE FROM ${USERS_TABLE} WHERE id = $1`, [targetId]);
...
// GET /api/admin/todos — 전체 todo + 작성자 이메일
app.get('/api/admin/todos', async (_req, res) => {
  try {
    const { rows } = await pool.query(
...
// DELETE /api/admin/todos/:id — 모든 todo 강제 삭제 가능
app.delete('/api/admin/todos/:id', async (req, res) => {
  try {
    ...
    const { rowCount } = await pool.query(`DELETE FROM ${TODOS_TABLE} WHERE id = $1`, [id]);
    if (rowCount === 0) {
